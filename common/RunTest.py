...
class TestSentenseResulver(unittest.TestCase):
    ...
    def test_split(self):
        ...
        self.assertListEqual(
            ...
            [u'(', u'abc', u'def', u')'])
        self.assertListEqual(
            self.tokenizer.split(u"()"),            # empty in brackets
            [u'(', u')'])
        self.assertListEqual(
            self.tokenizer.split(u"(abc def."),     # close bracket is missing
            [u'(', u'abc', u'def', u'.'])
        self.assertListEqual(
            self.tokenizer.split(u"abc def)."),     # open bracket is missing
            [u'abc', u'def', u')', u'.'])
        self.assertListEqual(
            self.tokenizer.split(u")("),            # wrong order
            [u')', u'('])
        self.assertListEqual(
            self.tokenizer.split(u")(abc)"),
            [u')', u'(', u'abc', u')'])
    ...
    def test_concatenate(self):
        self.assertTrue(
            ...
            "abc def")
        self.assertTrue(
            self.tokenizer.concatenate([u'<c1>', u'that', "isn't", 'cat', u'.', u'<c1>', u'that', u'is', u'a', u'dog', u'.']) ==
            "That isn't cat. That is a dog.")
        self.assertTrue(
            self.tokenizer.concatenate([u'abc', u'あ', u'い', u'う', u'def']) ==
            "abcあいうdef")
        self.assertTrue(
            self.tokenizer.concatenate([u'abc', u'def', u'あ', u'い', u'　', u'う', u'え', u'お']) ==
            "abc defあい　うえお")
        self.assertTrue(
